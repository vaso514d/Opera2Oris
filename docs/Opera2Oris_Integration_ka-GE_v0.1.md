...
BOF record გარდაიქმნება `OaTransactionRequest` მოდელად, რომელიც შეესაბამება `POST /api/Transaction`.
...
`OaTransactionEntryRequest` არის debit/credit entry.
...
კონვერტორი თითო BOF record-იდან ქმნის ორ entry-ს:

1. debit entry;
2. credit entry.

თუ თანხა უარყოფითია, debit/credit მხარეები იცვლება.
...
`Mapping`-ში მითითებული account-ები პირდაპირ განსაზღვრავს, რომელი Oris account მოხვდება OA transaction-ის debit/credit entry-ებში. converter არ აგზავნის BOF-ის raw account ნომრებს, რადგან Oris-ის chart of accounts კომპანიის მიხედვით იცვლება.

| Config field | როდის გამოიყენება | OA entry |
|---|---|---|
| `GuestLedgerAccount` | ყველა `Charge` ჩანაწერზე | debit account |
| `GuestLedgerAccount` | ყველა `Payment` ჩანაწერზე | credit account |
| `GuestLedgerAccount` | `Package` ჩანაწერზე, თუ `PackageLedgerAccount` ცარიელია | debit account fallback |
| `PackageLedgerAccount` | `Package` ჩანაწერზე, თუ შევსებულია | debit account |
| `DefaultRevenueAccount` | `Charge`/`Package` ჩანაწერზე, თუ `trx_code` და `tc_subgroup` mapping ვერ მოიძებნა | credit account fallback |
| `DefaultPaymentAccount` | `Payment` ჩანაწერზე, თუ `trx_code` და `payment_method` mapping ვერ მოიძებნა | debit account fallback |

Revenue account აირჩევა მხოლოდ `Charge` და `Package` ჩანაწერებისთვის. არჩევის პრიორიტეტია:

1. `RevenueAccountsByTransactionCode[trx_code]`
2. `RevenueAccountsByTransactionSubGroup[tc_subgroup]`
3. `DefaultRevenueAccount`
...
თუ თანხა უარყოფითია, converter ჯერ პოულობს debit/credit account-ებს ზემოთ აღწერილი წესებით, შემდეგ კი debit და credit მხარეებს უცვლის ადგილს. OA-ში თანხა იგზავნება absolute value-ით.

ცარიელი string-ები (`""`) და ცარიელი dictionary entry-ები არ გამოიყენება mapping-ში. ანუ `"DefaultRevenueAccount": ""` იგივეა, რაც value საერთოდ არ იყოს მითითებული.

თუ `DefaultRevenueAccount` ცარიელია:

- `Payment` ჩანაწერებზე ეს არ მოქმედებს, რადგან payment logic იყენებს payment account-ს და `GuestLedgerAccount`-ს.
- `Charge` და `Package` ჩანაწერები იმუშავებს მხოლოდ მაშინ, თუ შესაბამისი `trx_code` ან `tc_subgroup` mapping არსებობს.
- თუ `Charge`/`Package` record-ისთვის ვერ მოიძებნა არც `RevenueAccountsByTransactionCode`, არც `RevenueAccountsByTransactionSubGroup`, და არც `DefaultRevenueAccount`, OA payload არ შეიქმნება.
- ასეთ record-ზე log-ში დაიწერება warning: `Missing account mapping...`, record არ მოხვდება payload dump-ში, outbox-ში ან API upload-ში.

თუ სისტემას არ სჭირდება დეტალური revenue/payment account-ების გარჩევა transaction code-ებით, შესაძლებელია მარტივი default-account რეჟიმი:

```json
"Mapping": {
  "GuestLedgerAccount": "GE87ZZ0000000000000101",
  "PackageLedgerAccount": "GE60ZZ0000000000000102",
  "DefaultRevenueAccount": "GE33ZZ0000000000000200",
...
ამ რეჟიმში ყველა `Charge`/`Package` revenue მხარე წავა `DefaultRevenueAccount`-ზე, ხოლო ყველა `Payment` debit მხარე წავა `DefaultPaymentAccount`-ზე. ზემოთ მოცემული `GE...` მნიშვნელობები არის test IBAN-style placeholders და production Oris account-ებად არ უნდა ჩაითვალოს.
...
|---			|---		|---			|---																|
| `BOF_CD.csv` 	| `Charge` 	| `Daily` 		| ქმნის guest ledger debit და revenue credit transaction-ს 			|
...
| `BOF_KD.csv` 	| `Package` | `Daily` 		| ქმნის package/guest ledger debit და revenue credit transaction-ს 	|
...
| `BOF_PD.csv` 	| `Payment` | `Daily` 		| ქმნის payment account debit და guest ledger credit transaction-ს 	|
...
ანუ პირველი ასო (`C`, `K`, `P`) მოქმედებს debit/credit account-ის არჩევაზე. მეორე ასო (`D`, `O`, `U`) ამჟამად არ ცვლის accounting account mapping-ს; ის გამოიყენება audit/comment-ში, რომ ჩანაწერის წარმომავლობა გამოჩნდეს.

### გარდაქმნის ნაბიჯები

ერთი `BofExportRecord` გარდაიქმნება ერთ `OaTransactionRequest`-ად. ამ OA transaction-ში ყოველთვის იქმნება ორი entry:

1. debit entry;
2. credit entry.

თუ თანხა უარყოფითია, debit და credit account-ები იცვლება ადგილებით, ხოლო OA-ში იგზავნება თანხის absolute value. ეს საჭიროა, რომ reversal/refund ტიპის BOF ჩანაწერი სწორ მხარეს აისახოს.

თუ თანხა ნულია ან ვერ მოიძებნა, record არ იგზავნება API-ში და იწერება warning. თუ debit ან credit account ვერ მოიძებნა mapping config-ით, record ასევე არ იგზავნება API-ში.
...
- debit account არის `GuestLedgerAccount`;
- credit account არის revenue account.

ეს ნიშნავს: სტუმრის/დებიტორის დავალიანება იზრდება და შემოსავალი აღიარდება.

Package ჩანაწერებისთვის:

- debit account არის `PackageLedgerAccount`, თუ შევსებულია;
...
- debit account არის payment account, მაგალითად cash/bank/card account;
- credit account არის `GuestLedgerAccount`.

ეს ნიშნავს: ფული/ბარათი/ბანკი იზრდება და guest ledger-ის დავალიანება მცირდება.

Revenue account აირჩევა ამ პრიორიტეტით:

1. `RevenueAccountsByTransactionCode`
2. `RevenueAccountsByTransactionSubGroup`
3. `DefaultRevenueAccount`
...
- რამდენიმე BOF amount ველი ავსებს ერთ accounting amount-ს fallback პრიორიტეტით, ხოლო OA-ში ეს amount ორ entry-ად იშლება: debit და credit.
- `transactionComment` იღებს მხოლოდ CSV-ში არსებულ comment-like text-ს; source/audit context აღარ ემატება comment-ში.
...
| amount fields `113`, `10`, `63`, `62`, `65`, `64`, `60`, `61` | `transactionEntries[].debitAmount` და `creditAmount` | ყველა category | OA double-entry transaction-ს სჭირდება ერთი debit და ერთი credit თანხა |
...
| `trx_subgroup` ordinal `59` | revenue account lookup fallback | Charge/Package | subgroup გამოიყენება, როცა კონკრეტული transaction code არ არის mapping-ში |
...
| `description` ordinal `6` | `transactionComment` და `transactionEntries[].comment` | თუ შევსებულია | OA-ში ჩანს CSV transaction description, მაგალითად `VAT`, `Cash`, `Pre Stay Charge` |
...
| source file name და source line | outbox metadata და payload dump file name | ყველა category | audit/debug-ისთვის რჩება ტექნიკურ metadata-ში, მაგრამ OA comment-ში აღარ ემატება |
...
`transactionComment` აღარ იქმნება audit/context ტექსტის გაერთიანებით. comment-ში არ ემატება source file, line, category, room, guest name ან transaction code.

Comment აირჩევა CSV ველებიდან ამ პრიორიტეტით:

1. `description` ordinal `6`
2. `remark` ordinal `83`
3. `reference` ordinal `84`

იგივე value იწერება transaction header-ზე (`transactionComment`) და ორივე entry-ზე (`transactionEntries[].comment`).

```text
{description || remark || reference}
```

Audit ინფორმაცია source CSV-ზე და line-ზე ინახება outbox metadata-ში და payload dump file name-ში, მაგრამ Oris comment-ში აღარ იგზავნება.
...
6. ქმნის BOF batch-ს.
7. გარდაქმნის OA transaction payload-ებად.
8. თუ `PayloadDump.Enabled = true`, final JSON payload-ებს წერს dump folder-ში.
9. თუ `OaWebApi.UploadEnabled = true`, payload-ებს წერს durable outbox-ში.
...
2. ჩანაწერები გარდაიქმნება OA API payload-ებად.
3. payload-ები იწერება payload dump-ში ან outbox-ში.
...
`PayloadDump` გამოიყენება მაშინ, როცა OA API ჯერ არ არის მზად, მაგრამ გვჭირდება ვნახოთ ზუსტად რა final JSON გაეგზავნებოდა `POST /api/Transaction` endpoint-ს.
...
1. CSV იკითხება `BofExport.SourceDirectory`-დან.
2. BOF records გარდაიქმნება `OaTransactionRequest` payload-ებად.
3. თუ `PayloadDump.Enabled = true`, თითო payload იწერება JSON ფაილად `PayloadDump.Directory` folder-ში.
...
ფაილის სახელი შედგება source CSV-ის სახელისგან, line number-ისგან და `transactionDocumentNumber`-ისგან, მაგალითად:

```text
BOF_CD.csv_line000002_OPERA-104367130.json
```
...
3. ყოველი pending outbox record იგზავნება `POST /api/Transaction` endpoint-ზე.
...
Outbox-ში თითო transaction ინახება ცალკე JSON ფაილად.
...
- dump ფაილი არის API request body-ის ფორმაში: token და source metadata არ ინახება.
...
- `GuestLedgerAccount` - guest/debtor ledger account. გამოიყენება charge debit-ზე, payment credit-ზე და package debit fallback-ზე.
- `PackageLedgerAccount` - package debit account. თუ ცარიელია, package debit-ზე გამოიყენება `GuestLedgerAccount`.
- `DefaultRevenueAccount` - revenue fallback `Charge`/`Package` records-ისთვის. თუ ცარიელია და specific revenue mapping ვერ მოიძებნა, record skipped იქნება warning-ით.
