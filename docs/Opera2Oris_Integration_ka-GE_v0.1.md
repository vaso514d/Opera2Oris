...
| `DefaultRevenueAccount` | `Charge` / `Package` row-ზე, თუ specific revenue mapping ვერ მოიძებნა | revenue credit/debit entry |
| `DefaultVatAccount` | `Charge` / `Package` row-ზე, როცა gross და net სხვაობა VAT amount-ს ქმნის | VAT credit/debit entry |
...
VAT account აირჩევა parent charge/package row-ის VAT child row-იდან, თუ VAT row-ს აქვს CSV account value. თუ CSV VAT account ცარიელია, გამოიყენება `DefaultVatAccount`.
...
ანუ პირველი ასო (`C`, `K`, `P`) მოქმედებს entry set-ის logic-ზე: charge/package ქმნის sales entries-ს, payment ქმნის payment entries-ს. მეორე ასო (`D`, `O`, `U`) ამჟამად არ ცვლის account mapping-ს; ის გამოიყენება file category/scope-ის გასარჩევად.

### გარდაქმნის ნაბიჯები

ერთი `BofExportRecord` გარდაიქმნება ერთ `OaTransactionRequest`-ად. ამ OA transaction-ში იქმნება რამდენიმე `transactionEntries` item, accounting posting-ის მიხედვით.

`Charge` / `Package` row-ზე:

1. debtor/package account debit = gross amount;
2. revenue account credit = net amount;
3. VAT account credit = gross - net, თუ სხვაობა არსებობს.
...
- VAT entry account აირჩევა VAT child row-ის CSV account-იდან, შემდეგ `DefaultVatAccount`, როცა VAT amount არსებობს.
...
| amount fields `113`, `10`, `63`, `62`, `65`, `64`, `60`, `61` | `transactionEntries[].debitAmount` ან `creditAmount` | ყველა category | gross/payment amount ირჩევა fallback პრიორიტეტით და იშლება debit/credit entries-ად |
| `net_amount` ordinal `61` | revenue entry amount | `Charge` / `Package` row-ზე | revenue credit/debit amount მოდის net amount-იდან |
| `gross_amount - net_amount` | VAT entry amount | `Charge` / `Package` row-ზე, თუ სხვაობა არსებობს | VAT credit/debit amount გამოითვლება gross და net სხვაობით |
...
- `DefaultRevenueAccount` - revenue fallback `Charge` / `Package` records-ისთვის.
- `DefaultVatAccount` - VAT account. საჭიროა, როცა sales row-ზე `gross_amount - net_amount` არ არის ნული.
