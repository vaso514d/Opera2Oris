...
კონვერტორი თითო BOF record-იდან ქმნის ერთ entry-ს. JSON-ში ჩანს ორივე amount field:

- თუ თანხა დადებითია: `debitAmount = amount`, `creditAmount = 0`.
- თუ თანხა უარყოფითია: `debitAmount = 0`, `creditAmount = abs(amount)`.
...
თუ თანხა უარყოფითია, converter account-ს იგივე წესით პოულობს, მაგრამ amount გადადის `creditAmount` field-ში. OA-ში თანხა იგზავნება absolute value-ით.
...
თუ თანხა დადებითია, entry-ში იწერება `debitAmount = amount` და `creditAmount = 0`. თუ თანხა უარყოფითია, entry-ში იწერება `debitAmount = 0` და `creditAmount = abs(amount)`.
...
amount-ის მხარეს განსაზღვრავს თანხის ნიშანი: positive -> debit, negative -> credit.
...
- რამდენიმე BOF amount ველი ავსებს ერთ accounting amount-ს fallback პრიორიტეტით, ხოლო OA-ში ეს amount ერთ entry-ში იწერება debit ან credit მხარეს თანხის ნიშნის მიხედვით.
- `transactionComment` იღებს debtor/payee name-ს და CSV-ში არსებულ comment-like text-ს; source/audit context აღარ ემატება comment-ში.
...
| amount fields `113`, `10`, `63`, `62`, `65`, `64`, `60`, `61` | `transactionEntries[0].debitAmount` და `creditAmount` | ყველა category | დადებითი თანხა იწერება debit-ში, უარყოფითი თანხა credit-ში absolute value-ით |
...
| `payee_name` ordinal `20` | `transactionComment` და `transactionEntries[].comment` | თუ შევსებულია | debtor/payee name ჩანს OA comment-ში |
| `guest_name` ordinal `24` | `transactionComment` და `transactionEntries[].comment` fallback | თუ `payee_name` ცარიელია | guest name გამოიყენება debtor/payee name fallback-ად |
...
საბოლოო comment ფორმატია:

```text
{payee_name || guest_name} - {description || remark || reference}
```

თუ debtor/payee name ცარიელია, comment-ში დარჩება მხოლოდ comment text. თუ comment text ცარიელია, comment-ში დარჩება მხოლოდ debtor/payee name. იგივე value იწერება transaction header-ზე (`transactionComment`) და entry-ზე (`transactionEntries[0].comment`).
